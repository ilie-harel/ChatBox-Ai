...
function SpeechFromText() {
    ...
    async function stopListening() {
        ...
        const res = await apiService.sendMessageToChatGPT({ message: transcript, id: authSlice.sub, language: authSlice.language });
        if (res.status !== 200) {
            alert('error')
        } else {
            ...
            <div className="chatDiv">
                <div className="chat">
                    {messages && messages.map((m, index) => {
                        ...
                        return (
                            <div key={m.timestamp} className="chatMessage">
                                {m.role === 0 ?
